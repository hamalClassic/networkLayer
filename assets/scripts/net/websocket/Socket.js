...
var Socket = cc.Class({
    ...
    dispatch : function (binaryData, sz) {
        ...
        return (function () {
            if (! localConfig.messageNameInLogFilterList(name))
                cc.log("ws res name:", name, ", data:", JSON.stringify(data.args));
            return data
        })()
    },

    hostRequest : function (name, args, session) {
        let data = proto.hostRequest(name, args, session);
        return (function () {
            if (! localConfig.messageNameInLogFilterList(name))
                cc.log("ws req name:", name, ", data:", JSON.stringify(args));
            return data
        })()
    },
